...
def create_table(frame,graph,mainframe,window):
    ...
    def toggle_chart_shown():
       nonlocal chart_shown
       show_chart(graph, frame, window, month_select, not chart_shown)
       chart_shown = not chart_shown
...
def create_widgets(frame,treeview):
    ...
    save_value = ttk.Button(frame,text="Guardar",command=lambda: insert_row(expense_date,expense_type,expense_value,expense_cuota,expense_entry,treeview)) 
